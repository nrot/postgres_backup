pub mod wal {
    ...
    pub fn wal_copy(argv: ArgMatches) -> Result<(), Error> {
        ...
        let src = String::from(subcommand.value_of("source").expect("Source us required"));
        let filename = String::from(
            subcommand
                .value_of("filename")
                .expect("Filename is required"),
        );
        let dst = String::from(subcommand.value_of("dst_dir").expect("Dst dir is required"));
            ...
            if dst_file.exists() {
                ...
                if zip{
                    match File::open(src.clone()){
                        Ok(mut src_f)=>{
                            match File::create(dst_file.clone()){
                                Ok(back)=>{
                                    ...
                                    match zip.start_file(filename.as_str(), options){
                                        Ok(_)=>{
                                            ...
                                            match writed {
                                                ...
                                                Err(e) => {
                                                    msg.error = format!("Can`t write ZIP file: {e}", e=e);
                                                }
                                            }
                                ...
                                Err(e)=>{
                                    msg.error = format!("Can`t write backup file: {e}", e=e)
                                }
                            }
                        }, 
                        Err(e)=>{
                            msg.error = format!("Can`t read backup file: {e}", e=e)
                        }
                    }
                } else {
                    match std::fs::copy(src, dst_file.to_str().expect("Not empty filename")){
                        Ok(size)=>{
                            msg.orig_size = size as u64;
                        },
                        Err(e)=>{
